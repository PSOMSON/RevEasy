...
public class Reviser extends Vue implements AfficheurFiche {
    ...
    public void openFiche(Fiche f) {
        ...
        testFlashCards.addActionListener(e -> {
            startTestFlashCards(f);
        });
        ReviserText reviserQA = new ReviserText(f.getQuestions());
        questionAnswer.addActionListener(e -> {
            startQuestionAnswer(f, reviserQA);
        });
        ...
        testTexteAtrou.addActionListener(e -> {
            startTestAtROU(f, reviserText);
        });
    ...
    private void startTestFlashCards(Fiche fiche) {
        ...
        if (enonceList.size() == 0) {
            JOptionPane.showMessageDialog(null,
                    "La liste des " + (theoremeOUdefinition == 0 ? "Theoremes" : "Definitions") + " est vide.", "Alert",
    ...
    private void buttonClicked(ActionEvent e) {
        if (e.getSource() == revealButton) {
            ...
            enonceLabel.setText("<b>" + (theoremeOUdefinition == 0 ? "Théorème" : "Definition") + " : </b><br>"
                    + enonceList.get(currentEnonceIndex).getTitre());
