    ...
    private void next() {
        if (!res.getText().isEmpty()) {
            ...
            if (guess.equalsIgnoreCase(originalTexts.get(currentIndex))) {
                JOptionPane.showMessageDialog(res, "Bravo! Votre réponse est bonne.",
                "Succ�s", JOptionPane.INFORMATION_MESSAGE);
            } else {
                JOptionPane.showMessageDialog(res,
                        "Désolé, votre supposition était incorrecte.",
            ...
            for (int j = 0; j < rs.get(i).size(); j++) {
                if (rs.get(i).get(j).isSelected()) {
                    if (originalQA.get(i).estJuste(rs.get(i).get(j).getText())) {
                        JOptionPane.showMessageDialog(res, "Bravo! Votre " + count
                                + " r�ponse est bonne.",
                                "Succ�s", JOptionPane.INFORMATION_MESSAGE);
                    } else {
                        JOptionPane.showMessageDialog(res, "Désolé, votre " + count
                                + " supposition était incorrecte.",
                                "Incorrecte", JOptionPane.ERROR_MESSAGE);
                    }
                    count++;
                }
